...
description: >-
  ...
  .omc/state/, and conducts a back-and-forth Clarifying → High Level → Drill
...
```bash
echo -n "$TOPIC" > .omc/state/active-sysdesign-topic.txt
```
...
Read `.claude/skills/sysdesign-question-bank/problems/<slug>.md` (the matched
...
1. **Back-of-envelope.** If `sysdesign-frameworks/capacity-estimation.md` was
   ...
   apply formulas. If not, ask Claude (you) to read it on demand.
...
DO NOT touch `.omc/state/active-sysdesign-topic.txt` — keep the topic active so
...
active (`.omc/state/active-sysdesign-topic.txt` already populated):
