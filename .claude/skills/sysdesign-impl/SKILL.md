...
```bash
TOPIC=$(cat .omc/state/active-sysdesign-topic.txt 2>/dev/null)
```
...
Default stack (per `.claude/CLAUDE.md` skills bias toward Spring Boot):
...
If existing `.claude/skills/new-feature` or `domain-modeling` / `layer-architecture`
skills apply (Spring Boot + DDD + TDD), follow their conventions. Otherwise apply
project-wide conventions from `.claude/CLAUDE.md`:
...
토픽을 다른 토픽으로 갈아탈 준비가 됐다면 `.omc/state/active-sysdesign-topic.txt`
...
| `<topic>/source/` 외 다른 곳 (예: 다른 토픽 dir, .claude/) 절대 안 건드림. | per-topic isolation. |
