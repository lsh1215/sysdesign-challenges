...
This repository is a practice workspace for the cycle:

1. Human-led system design in `<topic>/System-Design-Document/`
2. AI-assisted minimum viable implementation in `<topic>/source/`
3. Verification through functional, load, and failure tests in `<topic>/test-results/`

Default implementation stack is **Spring Boot 3.x + Java 21** unless the user explicitly asks for another stack.
...
This project was migrated from Claude Code. The old `.claude/` directory may remain for history, but new Codex-oriented guidance lives here and in `.codex/skills/`.

Claude hooks and custom sub-agent routing do not run in Codex. When a user request matches one of the skill trigger areas below, read the matching `.codex/skills/<skill>/SKILL.md` file before planning or editing. For `sysdesign-frameworks`, read only the index first, then load the specific referenced framework file needed for the decision.

## Default Routing

For implementation requests where the stack is not specified:

1. Use Spring Boot 3.x + Java 21.
2. For Spring Boot / Java code work, prefer the `spring-boot-expert` native agent/prompt from `.codex/agents/spring-boot-expert.toml` and `.codex/prompts/spring-boot-expert.md` when native agent routing is available.
3. If `<topic>/source/` is empty or has no `build.gradle`, read `.codex/skills/spring-bootstrap/SKILL.md` and scaffold from its templates.
4. Implement in this order: design update, domain model, tests, application service, tests, controller, tests.

If the user explicitly asks for Python, Node, Go, or another stack, do not use the Spring bootstrap path. Still carry over the architecture concepts: DDD layers, DTO separation, TDD, centralized error handling, and clear transaction boundaries.

## Core Engineering Rules

- Keep each topic isolated. Do not share source code across topic directories.
- Update the design document before code when implementation changes the design.
- Use TDD for meaningful code: RED, GREEN, REFACTOR. Do not add untested production behavior.
- Use layered architecture: `api -> application -> domain -> infra`.
- Domain code must not depend on infrastructure.
- API DTOs live under `api/dto/`; internal command/result DTOs live under `application/dto/`.
- Prefer Mermaid or ASCII diagrams so design changes are diffable.
- Avoid AI-looking artifacts: TODO filler comments, generated-watermark text, excessive docstrings, and obvious boilerplate explanation comments.
- Commit messages should be `<topic>: <change>` for topic work, or conventional commits such as `docs:`, `feat:`, `fix:`, `refactor:`, `test:`, `chore:`.

## File Access Rules

- Never read `.env` files.
- `.env.example` and `.env.sample` are allowed.
- Codex/OMX active topic state lives at `.omx/state/active-sysdesign-topic.txt`.

## Spring Boot Rules

For Spring Boot work, follow `.codex/skills/spring-exception-handling/SKILL.md`, `.codex/skills/layer-architecture/SKILL.md`, `.codex/skills/domain-modeling/SKILL.md`, and `.codex/skills/tdd-patterns/SKILL.md` as applicable.

Hard rules:

- No broad `try { ... } catch (Exception e) { ... }` in business or controller code.
- Throw `BusinessException` or a domain-specific subclass and let `GlobalExceptionHandler` translate it.
- Use constructor injection with `@RequiredArgsConstructor` and `private final` fields.
- Do not leak entities across controller boundaries.
- Put `@Transactional` on application service public methods only.
- One transaction should mutate one aggregate.

Before editing Spring code:

1. Read the nearest existing files of the same type.
2. Read shared kernel classes if present: `BusinessException`, `ErrorCodeBase`, `ApiResponse`, `BaseEntity`, `GlobalExceptionHandler`.
3. Match existing package and test style.

## Skills

Read these on demand from `.codex/skills/`:

| Skill | Use When |
|---|---|
| `domain-modeling` | DDD, aggregate roots, value objects, domain events, bounded contexts |
| `layer-architecture` | REST APIs, controllers, services, DTOs, transaction boundaries |
| `tdd-patterns` | Unit/integration/controller/repository tests and TDD workflow |
| `spring-exception-handling` | Spring exception handling, `BusinessException`, `GlobalExceptionHandler` |
| `spring-bootstrap` | New Spring Boot multi-module scaffold under `<topic>/source/` |
| `new-feature` | New domain feature implemented in TDD order |
| `event-driven` | Kafka, outbox, idempotent consumers, event naming |
| `saga-pattern` | Distributed transaction orchestration and compensation |
| `e2e-testing` | Browser or end-to-end QA workflow |
| `sysdesign-design` | Start or continue a mock interview style system design |
| `sysdesign-sdd` | Promote mock interview notes into a formal SDD |
| `sysdesign-impl` | Scaffold a minimum viable implementation from a design |
| `sysdesign-frameworks` | Capacity estimation, latency numbers, CAP, caching, rate limiting, hashing, fan-out, DB choice |
| `sysdesign-question-bank` | Alex Xu Vol.1 topic catalog and canned problem numbers |

## System Design Workflow

For a new design topic:

1. Resolve the topic slug using `.codex/skills/sysdesign-question-bank/SKILL.md`.
2. Create `<topic>/System-Design-Document/diagrams`, `<topic>/source`, and `<topic>/test-results`.
3. Start from the appropriate template in `templates/System-Design-Document/`.
4. Write decisions into `mock-interview.md` immediately as the conversation progresses.
5. Use `.codex/skills/sysdesign-frameworks/` only for the specific framework being discussed.

For SDD promotion:

1. Prefer `<topic>/System-Design-Document/mock-interview.md` as the source.
2. Use `templates/System-Design-Document/software-design-document-style.md` as the structure.
3. Ask only for missing SDD-specific decisions such as constraints, ADRs, risk, rollout, and testing strategy.

For implementation:

1. Prefer `<topic>/System-Design-Document/sdd.md`.
2. Fall back to `mock-interview.md` only after stating the missing SDD limitation.
3. Build the smallest runnable system that verifies the design assumptions.
4. Exclude production-only scale components such as multi-region, CDN, replication, and large clusters unless they are directly testable in the MVI.
